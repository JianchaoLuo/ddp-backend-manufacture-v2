 ...
 * @ClassDescription:
 * @Author: KangHong
 * @Created: 2024/12/23 21:36
 */
@Data
@Slf4j
public class WholeSimulation_2 {
    /**
     * 仿真处理合箱区之前输入的工具类
     */
     ...
     * 测试的模拟其他区域类
     */
    private OtherSimulation otherSimulation = new OtherSimulation();
    ...
    public SimulateResult simulationRun(SimulateInput input){
        ...
        //创建造型线的仿真
        MouldingSimulation mouldingSimulation = new MouldingSimulation();
        //创建合箱区的仿真
        ...
        //创建开箱区的仿真
        OpenBoxSimulation_2 openBoxSimulation = new OpenBoxSimulation_2();
...
//        log.info(productNum + "?" + size);
        ...
        while (true){
            try {
                ...
                //获得造型线仿真结果
                //更新仿真输入
                outBoundMouldingAndClosingOutPut = mouldingSimulation.MouldingSimulationRun(outBoundMouldingAndClosingBox, timeList, input.getAnalogInput());
                ...
                //模拟其他区域的仿真
                //更新仿真输入
                otherSimulationLink = otherSimulation.otherSimulationRun(outBoundANDMoldClosing.getOutBoundGanttChartLinkInput(), outBoundANDMoldClosing.getMoldClosingGanttChartLinkInput(),
                ...
                if (waitingPourCount == productNum){
                    ...
                    if (addEventFlag){
                        afterClosingBoxAllLink.getWaitingPourInput().getGanttChart().getEventLinkGantts().get(afterClosingBoxAllLink.getWaitingPourInput().getInput().getWaitingPourIndexInput().getClosingBoxGoPourSubCarInGanttIndex()).getEvents().add(allTool.createFlagEvent());
        ...
        SimulateResult simulateResult = new SimulateResult(charts, orderTimes);
