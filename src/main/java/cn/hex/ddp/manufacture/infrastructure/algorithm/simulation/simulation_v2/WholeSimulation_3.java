...
import cn.hex.ddp.manufacture.infrastructure.algorithm.simulation.*;
...
@Data
@Slf4j
public class WholeSimulation_3 {
    /**
     * 仿真处理合箱区之前输入的工具类
     */
    ...
    public SimulateResult simulationRun(SimulateInput input){
        ...
        //创建出库区的仿真
        OutBoundSimulation_3 outBoundSimulation_3 = new OutBoundSimulation_3();
        ...
        //创建开箱区的仿真
        OpenBoxSimulation_3 openBoxSimulation = new OpenBoxSimulation_3();

        //根据新的工厂流程，修改生成的出库队列
        productDeque = handleInput.updataProductDeque(productDeque);
        ...
        //初始化冷却区的仿真输入
        CoolingGanttChartLinkInput coolingGanttChartLinkInput = pourHandleInput.handleCoolingInput(input, totalTime,
                SerializationUtils.clone(waitingPourGanttChartLinkInput.getInput().getClosingGoPourPath()));
        ...
        //用于造型线仿真输入输出
        OutBoundMouldingAndClosingBox outBoundMouldingAndClosingBox = new OutBoundMouldingAndClosingBox(outBoundGanttChartLinkInput,
                mouldingGanttChartLinkInput, closingBoxGanttChartLinkInput);
        OutBoundMouldingAndClosingBox outBoundMouldingAndClosingOutPut;

        //用于待浇筑区之后仿真输入输出
        AfterClosingBoxAllLink afterClosingBoxAllLink = new AfterClosingBoxAllLink(closingBoxGanttChartLinkInput, waitingPourGanttChartLinkInput,
                pouringGanttChartLinkInput, coolingGanttChartLinkInput, openBoxGanttChartLinkInput, outBoundGanttChartLinkInput,
                waitingPourCount, upImitates, downImitates);
        AfterClosingBoxAllLink afterClosingBoxAllOutPut;

        //其他区域仿真模拟
        OtherSimulationLink otherSimulationLink;
        //用于测试
        ...
        for (int i = 0; i < input.getOrderInputs().size(); i++) {
            for (int j = 0; j < input.getOrderInputs().get(i).getOrderProducts().size(); j++) {
                productNum = productNum + (input.getOrderInputs().get(i).getOrderProducts().get(j).getNumber() -
                        input.getOrderInputs().get(i).getOrderProducts().get(j).getFinishNum());
            }
        }
...
//        log.info(productNum + "?" + size);
        ...
        while (true){
            try {
                //获得出库区仿真结果
                //更新仿真输入
                outBoundANDOutPut = outBoundSimulation_3.OutBoundSimulationRun(outBoundANDMoldClosing.getOutBoundGanttChartLinkInput().getInput(),
                ...
                //模拟其他区域的仿真
                //更新仿真输入
                otherSimulationLink = otherSimulation_2.otherSimulationRun(outBoundANDMoldClosing.getOutBoundGanttChartLinkInput(),
                        outBoundANDMoldClosing.getMoldClosingGanttChartLinkInput(), outBoundMouldingAndClosingBox.getMouldingInput(),
                        outBoundMouldingAndClosingBox.getClosingBoxInput(), dTime, finishedCount, input.getAnalogInput());
                ...
                if (waitingPourFlag){
                    ...
                    if (afterClosingBoxAllLink.getOpenBoxInput().getInput().getCastingShippingPosition().getStatus().equals(PositionStatusEnum.OCCUPIED)){
                        ...
                        afterClosingBoxAllLink.getOpenBoxInput().getGanttChart().getEventLinkGantts().get
                                (afterClosingBoxAllLink.getOpenBoxInput().getInput().getOpenBoxIndexInput().getPositionInGanttIndex()).getEvents().add(event);
                    }
                }

                if (finishedCount == productNum * 4){
                    break;
                }
